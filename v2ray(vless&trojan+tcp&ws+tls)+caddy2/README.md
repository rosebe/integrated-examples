...
v2ray 通过配置相关参数对 vless+tcp、trojan+tcp 进行端口分流（四层转发），实现共用443端口。vless+tcp 以 h2 或 http/1.1 自适应协商连接，分流 ws（WebSocket）连接，非 v2ray 的 web 连接回落给 caddy2。同时 trojan+tcp 也以 h2 或 http/1.1 自适应协商连接，非 v2ray 的 web 连接回落给 caddy2。v2ray 包括应用如下：
