...
4、因 v2ray SNI 分流不支持 PROXY protocol（发送），故不启用此项应用。

5、因 trojan(trojan-go) 不支持 PROXY protocol（接收），而 nginx SNI 中的 PROXY protocol 发送是针对共用端口全局模式，故 v2ray SNI 分流时不能启用此项应用。
