...
v2ray 通过配置相关参数对 v2ray vless+tcp、v2ray trojan+tcp 进行端口分流（四层转发），实现共用443端口。配置7实现了 vless tcp 以 http/1.1 或 http/2 自适应代理科学上网，分流出 ws，回落给 nginx。同时 v2ray trojan（trojan+tcp）也以 http/1.1 代理科学上网，回落给 nginx。v2ray 包括应用如下：
