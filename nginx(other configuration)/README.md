...
3、也可以用 haproxy SNI 分流 v2ray 回落应用与网站应用来解决共存问题。但若已采用 nginx 来做回落 web 服务器，不推荐（无必要再增加软件）；若采用非 nginx 来做回落 web 服务器，推荐采用 haproxy SNI 分流解决（配置示例参考本人 haproxy SNI 分流示例）。
