...
此配置包括v2ray、naiveproxy(caddy2)应用。v2ray、naiveproxy(caddy2)各自公开一个监听端口，各自分别或配合提供服务。另caddy2还同时为v2ray（vless+tcp+tls与trojan+tcp+tls）提供回落服务。v2ray包括如下应用：
...
v2ray tcp类应用直连，v2ray ws类应用分流一次；v2ray trojan分流两次次；naiveproxy直连，v2ray h2类应用分流（反代）一次。
